...
PROJ_DIR="apms_setup" # Project location
PROJ_NAME="ApolloMusicStrips" # Project abbreviation
...
function usage {
    ...
    echo ""
    prompt -i "OPTIONS"
    prompt -i "  -b, --branch        git branch to use (main, dev_2.3)"
...
if [[ -d $PROJ_DIR ]]; then
    if [[ true ]]; then
        ...
        sudo git clone --depth 1 --branch $GIT_BRANCH https://github.com/ignotus321/apms_setup.git
...
Type=simple
User=root
WorkingDirectory=${INST_DIR}/apms_setup/server
